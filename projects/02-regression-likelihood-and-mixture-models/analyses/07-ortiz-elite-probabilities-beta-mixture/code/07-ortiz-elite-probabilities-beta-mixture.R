# Project 02 - Analysis 07
# David Ortiz elite probabilities under the normal / beta mixture
#
...
if (!requireNamespace("Lahman", quietly = TRUE)) {
  stop("Analysis 07 requires the Lahman package. Install it with install.packages(\"Lahman\").")
}
...
# For the full version, increase this back to 30 if desired.
n_starts <- 5
fits <- vector("list", n_starts)

cat("Analysis 07: fitting Normal/Beta mixture for Ortiz probabilities.\n")
...
ortiz$p_elite <- elite_prob_normal_beta(
  ortiz$HRrate,
  best$pi,
  best$mu,
  best$sigma,
...
write.csv(
  start_results,
  file.path(results_dir, "em_start_results_beta_mixture.csv"),
  row.names = FALSE
)

write.csv(
  fit_used,
  file.path(results_dir, "normal_beta_fit_parameters_used.csv"),
  row.names = FALSE
)

write.csv(
  ortiz_out,
  file.path(results_dir, "ortiz_elite_probabilities_beta_mixture.csv"),
  row.names = FALSE
)

png(
  file.path(fig_dir, "ortiz_elite_probabilities_beta_mixture.png"),
  width = 900,
  height = 700
)
...
plot(
  ...
  xlab = "Season",
  ylab = "P(elite | HR/AB)",
  main = "David Ortiz elite probabilities: Normal/Beta mixture"
)
